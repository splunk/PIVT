...
func (s *SmartContract) update(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	// Set an event.
	setEventErr := APIstub.SetEvent("update-event", []byte{'h', 'e', 'y', ' ', 'y', 'o', 'u'})
	if setEventErr != nil {
		return shim.Error(fmt.Sprintf("Could not set event for %s in the ledger: %s", name, setEventErr.Error()))
	}
